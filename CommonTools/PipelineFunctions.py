...
def load_params(analysis_folder):
    ...
    f = open(analysis_folder+os.sep+r'experiment_params.txt')
    for idx,line in enumerate(f):
        
        # experiment name
        if idx == 0:
            exp_name = str(line).split(' = ')[-1][:-1]
        # experiment default threshold
        elif idx == 1:
            exp_threshold = float(line.split(' = ')[-1][:-1])
        else:
            continue
    
    f.close()
    
    return exp_name, exp_threshold
...
def load_dapi_dax_frame(h0_folder, fovs, frame):
    ...
    #ims = [io.DaxReader(h0_folder+os.sep+fov).loadAll() for fov in fovs]#10*5-1
    #return [ np.max(im, axis=0) for im in ims ]
...
def flat_field_correction(folders, analysis_folder, fovs, colors, force_rerun=False):
        ...
        if not np.all([os.path.exists(fl) for fl in dax_fls]) or force_rerun:#True
            ...
            for icol,dax_fl in enumerate(dax_fls):
                ...
                if not os.path.exists(dax_fl) or force_rerun:
                    for idax in tqdm(inds):
                        ...
                        if idax%ncolors_signal==icol: #this keeps only the 750 color
                            if np.max(np.abs(tzxy1-tzxy2))<=1: #keep if drift error is within a pixel
                                ...
                                im = ft.translate(im,-tzxy1[[0,2,1]])
                    ...
                    #filename = analysis_folder+os.sep+r'a750_38.dax'
                    #dax = DaxFile(filename, num_channels=1)
                    #data_scaled = dax.get_entire_image()
                    #data_scaled = np.swapaxes(data_scaled,1,2)
                    im_mean_save = im_mean/float(nim)
                    im_mean_save = im_mean_save[:,:,:]#-data_scaled

                    np.save(dax_fl.replace('.dax','_min-max.npy'),[np.min(im_mean_save),np.max(im_mean_save)])
                    io.writeDax(im_mean_save,dax_fl)

def get_candidate_spots(analysis_folder, ims_segmented, colors):
    ...
    for dax_round in tqdm(list(zip(*daxs))):
        ...
        for dax in dax_round:
            ...
            # normalize the brightness for im_all according to the min and max values computed
            # for each FOV separately, averaging across all rounds.
            im_mn = 1.*im_all/np.max(im_all)*max_+min_
            im_mns.append(im_mn)
            filt_im = im_mn#median_filter(im_mn, size=5)
            zxyhs.append(ft.return_candidate_centers(filt_im,ex=20,ey=20,ez=2,gmax=10,filtsz=8,ths=2,
                                      plt_val=False,save_file=None))

        saveloc = analysis_folder+os.sep+r'candidates_before_top_two.npy'
        np.save(saveloc, zxyhs)
        #
        im_segm = ims_segmented[isegm]#cv2.resize(ims_segmented[isegm],tuple(im_mn.shape[1:]),interpolation  = cv2.INTER_NEAREST)

        # here we find at least two chrs per cell and up to 5 based upon brightnesses
        # this only looks at spots identified in the return_candidate_centers above.
        zxyh_keep, cell_ids = ft.get_N_chrs_per_cell(im_segm, isegm, np.concatenate(zxyhs,axis=-1), dist = 20)
        isegm+=1
        ...
        if (len(zxyh_keep) > 0):
            ...
            plt.contour(im_segm,[0.5+x for x in range(np.max(im_segm))])
    ...
    cell_id_fls = glob.glob(analysis_folder+os.sep+'*__cell_ids.npy')
    ...
    for coord in coords:
        if len(coord) > 0:
            dic_coords.append(coord[:,[1,2,0]])
    dic_save['coords'] = ft.flatten(dic_coords)
...
def drift_correction(fov_id, folders, analysis_folder, colors):
    ...
    th_fluctuations = 4 # 2 std away for selecting a spot

    ##RUN
    print(fov_id)
    fov_idx = np.where(fov_ids == fov_id)[0][0]
    ...
    file_=fovs[fov_id]
    chr_pts_ = chr_pts[fov_idx] #positions of the selected centers of chromosomes in reference frame.
...
def drift_correction_batch(folders, fovs, analysis_folder, colors, jobs=8):
    import multiprocessing
    import functools

    dic = pickle.load(open(analysis_folder+os.sep+'Selected_Spot.pkl','rb'))
    drift_partial = functools.partial(drift_correction, folders=folders, analysis_folder=analysis_folder, colors=colors)
    with multiprocessing.Pool(jobs) as pool:
        pool.map(drift_partial, fovs)
        
    return None

def em_algorithm(analysis_folder, colors, regions, blank_rounds=[], chromatic_correction=None, use_center=1):
    ## Read files ############
    ...
    fovs_spots = np.array(ft.flatten(dic_cell_ids))//1000
    saveloc = analysis_folder+os.sep+r"fov_ids_em_alg"
    np.save(saveloc, np.array(fovs_spots))
    ...
    #skip reprobe rounds for now, maybe remove this later.
        if 'RR' in htag:
            continue
        col = htag.split('_')[-1]
    ...
    ### Initialize ########
    zxys_f,hs_f,cols_f,snrs_f  = [],[],[],[]

    print("Initializing")
    
    #chck number of parameters stored by 'ps' array in dictionary of spots to see if we have snr.
    no_snr_present = False
    for dic_spot in dic_spots_cor:
        for isp in dic_spot.keys():  # isp is a chromsomes in a single FOV
            dic_cand = dic_spot[isp][0]
            if len(dic_cand[htag]['ps']) == 0:
                continue
            elif len(dic_cand[htag]['ps'][0]) == 8:
                no_snr_present = True
            break
        break

    # check if the snr parameter is present.
    if no_snr_present:

        zxys_f, hs_f, cols_f, scores_f, all_scores_f = em_alg_no_snr(analysis_folder, fovs_spots, dic_spots_cor, htags, blank_rounds, use_center)
        return zxys_f, hs_f, cols_f, dic_spots, scores_f, all_scores_f, dic_spots_cor, dic_drifts, dic_cell_ids, fovs_spots

    for dic_spot in tqdm(dic_spots_cor):
        for isp in dic_spot.keys(): # isp is a chromsomes in a single FOV
            dic_cand = dic_spot[isp][0] # this is the dictionary of all rounds with params for all candidates in each
            zxys_T,hs_T,cols_T,snrs_T = ft.get_maxh_estimate(dic_cand,htags) # initlaize with brightest spots (max height)
            zxys_f.append(zxys_T)
            hs_f.append(hs_T)
            cols_f.append(cols_T)
            snrs_f.append(snrs_T)

    ### Run to converge #########
    def refine_set(dic_spots,zxys_f,hs_f, blank_rounds=[], use_local=1,use_center=1,use_snr=1,use_brightness=1):

        # we omit any rounds where we won't expect fluorescence from all chrs here to avoid biasing toward bad data.
        zxys_f_blanks_removed = copy.deepcopy(zxys_f)
        for rnd in blank_rounds:
            del(zxys_f_blanks_removed[rnd])
        # this is where we gather the currently accepted points and sort them for each category.
        Dhs,Ddists,DdistsC,Dsnrs = ft.get_Ddists_Dhs_Dsnrs(zxys_f_blanks_removed,hs_f,snrs_f,nint=5)
        zxys_f2,hs_f2,cols_f2,scores_f2,snrs_f2,all_scores_f2  = [],[],[],[],[],[]
        i_ = 0
        ...
        for dic_spot in tqdm(dic_spots): # for each FOV
            for isp in dic_spot.keys(): # for each spot
                dic_cand = dic_spot[isp][0]
                zxys_T,hs_T,cols_T,scores_T,snrs_T,all_scores = ft.get_statistical_estimate(dic_cand,htags,Dhs,Ddists,DdistsC,Dsnrs,
                                         zxys_T=zxys_f[i_],nint=5,use_local=use_local,use_center=use_center,use_snr=use_snr,use_brightness=use_brightness) # for each round
                zxys_f2.append(zxys_T)
                hs_f2.append(hs_T)
                cols_f2.append(cols_T)
                scores_f2.append(scores_T)
                snrs_f2.append(snrs_T)
                all_scores_f2.append(all_scores)
                i_+=1

        return zxys_f2,hs_f2,cols_f2,scores_f2,snrs_f2,all_scores_f2
    ...
    #for num_ref in range(niter):
        ...
        use_local = 1 if num_ref<niter/2 else 0
        use_center = use_center
        use_snr = 1
        use_brightness = 1
        zxys_f,hs_f,cols_f,scores_f,snrs_f,all_scores_f = refine_set(dic_spots_cor,zxys_f,hs_f,blank_rounds,use_local=use_local,use_center=use_center,use_snr=use_snr,use_brightness=use_brightness)
    ...
    return zxys_f, hs_f, cols_f, snrs_f, scores_f, all_scores_f, dic_spots_cor, dic_drifts, dic_cell_ids, fovs_spots

'''
This will run if we have data running through the pipeline that is old and does not contain SNR in the
parameter array from the fitting step of the algorithm.
'''
def em_alg_no_snr(analysis_folder, fov_ids, dic_spots_cor, htags, blank_rounds=[], use_center=1):
    ...
    for dic_spot in tqdm(dic_spots_cor):
        for isp in dic_spot.keys():
            dic_cand = dic_spot[isp][0]
            for idx, key in enumerate(dic_cand.keys()):
                all_candidates.append([chr_num, fov_ids[chr_num], idx, dic_cand[key]])
            chr_num += 1
            zxys_T,hs_T,cols_T = ft.get_maxh_estimate_no_snr(dic_cand,htags)
            zxys_f.append(zxys_T)
            hs_f.append(hs_T)
            cols_f.append(cols_T)

    savename = analysis_folder+os.sep+r'for_blair_intermediary.pkl'
    pickle.dump(all_candidates, open(savename, 'wb'))

    ### Run to converge #########
    def refine_set_no_snr(dic_spots,zxys_f,hs_f, blank_rounds=[], use_local=True,use_center=use_center):
        ...
        Dhs,Ddists,DdistsC = ft.get_Ddists_Dhs(zxys_f_blanks_removed,hs_f,nint=5)
        ...
        for dic_spot in tqdm(dic_spots): # for each FOV
            for isp in dic_spot.keys(): # for each spot
                dic_cand = dic_spot[isp][0]
                zxys_T,hs_T,cols_T,scores_T,all_scores = ft.get_statistical_estimate_no_snr(dic_cand,htags,Dhs,Ddists,DdistsC,
    ...
    for num_ref in range(niter):
        ...
        zxys_f,hs_f,cols_f,scores_f,all_scores_f = refine_set_no_snr(dic_spots_cor,zxys_f,hs_f,blank_rounds,use_local=use_local)
        saved_zxys_f.append(zxys_f)
        save_hs_f.append(hs_f)
    ...
    return zxys_f, hs_f, cols_f, scores_f, all_scores_f

# remove any chrs which are not the best two in a given cell. This is done solely
# based upon of the number of nan values present.
def top_two_chrs(chrs, cell_ids):

    keep = []

    if len(chrs) != len(cell_ids):
        print("Cell ID assignments are not the same length as the coordinates.")
        return None


    # go through all cell ids.
    for idx,cid in enumerate(np.unique(cell_ids)):

        # take the chrs associated with the given cell
        inds = np.where(cell_ids == cid)[0]
        if len(inds) < 2:
            continue

        candidates = chrs[inds]
        num_nans = [ sum(np.isnan(candidates[i,:,0])) for i in range(len(candidates)) ]# sum all nan values in a chromosome
        # find the values with the fewest nans and then pull top 2 index.
        top2 = np.argsort(num_nans)[-2:]

        # keep the chrs located at the top2 index locations.
        keep.extend(inds[top2])

    return chrs[keep], cell_ids[keep], keep


def filter_chromosomes(num_segs, max_missing, zxys_f, hs_f, cols_f, scores_f, all_scores_f, dic_cell_ids, fovs_spots, cutoff_exp, stdev_outlier=4):
    ...
    # remove any points which are more than stdev_outlier standard deviations from the centeroid of a given trace.
    ...
    removed = 0
    total = 0
    for trace in zxys_clean:
        
        # find median position 
        z_median, x_median, y_median = centeroidnp(trace)
        
        dists = [ np.linalg.norm(x-np.array([z_median, x_median, y_median])) for x in trace ]
        stdev = np.nanstd(dists)
        trace[dists>(stdev_outlier*stdev)] = np.array([np.nan,np.nan,np.nan])
        removed += sum(dists>(stdev_outlier*stdev))
        total+=num_segs
    
    
    print("Removed {} points of a total {} due to concerns of them being outliers.".format(removed, total))
    ...
    cell_ids_clean = np.array(ft.flatten(dic_cell_ids))
    cell_ids_clean = cell_ids_clean[keep]
...
def make_post_analysis_folder(analysis_folder, name='Post_analysis'):
    post_analysis_folder = analysis_folder+os.sep+name+os.sep
...
def get_drift_dic(analysis_folder,fov,htags):
    ...
    txyzs = np.array(pickle.load(open(drift_fl,'rb'), encoding='latin1'))
...
def plot_grid_ims_group(zxys_clean_, fovs_spots_keep, master_folders, scores_clean, analysis_folder, colors, aratio=2,
    ...
    for idx, ichr in enumerate(chrs):
        ...
        ims_3d,zxys_im = return_ims_pts(zxy, fov, readouts=None, master_folder=master_folders[0],
                          analysis_folder=analysis_folder, cols=colors, pix_size=[200,109,109],window=20)
...
'''
def brightness_by_color(zxys_f, master_folders, analysis_folder, fov_ids, colors, rnds=42):
    ...
    for fov_number in tqdm(np.unique(fov_ids)):
        ...
        signal_dax_list, signal_color_tags, bead_dax_list, bead_color_tags = io.get_ims_fov_daxmap(folders_keep, fov_filename, col_tags = colors, pad = 0)
    ...
    colors_ids= np.array(colors_ids)
    ...
    colors_no_nan = np.array(colors_ids)[(np.where(np.logical_not(np.isnan(brights)))[0])]
...
if __name__ == "__main__":
    ...
    drift_correction_batch(folders, analysis_folder, colors, jobs=25)
