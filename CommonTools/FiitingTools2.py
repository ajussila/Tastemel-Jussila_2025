...
def fast_local_fit(im,centers,radius=5,width_zxy=[1,1,1],return_good=False):
    ...
    for center in centers:
        ...
        # check if there is a good fit. If there was, change the last entry to be True.
        if p is not None:
...
def get_seed_points_base_v2(im_sm,gfilt_size=5,filt_size=3,th_seed=3.,max_num=None):
    ...
    """
    if gfilt_size!=0: im_norm = normalzie_im(im_sm,gfilt_size) # create a gaussian filtered image
    else: im_norm=im_sm
    std_ = np.nanstd(im_norm) # find stdev for the gaussian filtered image
    hcutoff = std_*th_seed # find the brightness for the th_std cutoff
    #
    z,x,y = np.where(im_norm>hcutoff) # finay any positions where the brightness exceeds this value.
    h_im = im_norm[z,x,y] # grab brightnesses of the points identified here.
    sz,sx,sy = im_norm.shape
    keep = h_im>0 # only keep spots with nonzero brightness
    ...
    for deltax in deltas:
        for deltay in deltas:
            for deltaz in deltas:
                # keep only spots which are above h_im im brightness
                keep &= (h_im>=im_norm[(z+deltaz)%sz,(x+deltax)%sx,(y+deltay)%sy])

    zf,xf,yf = z[keep],x[keep],y[keep] # keep only the spots identified.
    hf = im_norm[zf,xf,yf] # keep the heights of the selected spotss
    inds = np.argsort(hf)[::-1] # get the indices of the sorted values.
    centers_zxyh = np.array([zf[inds],xf[inds],yf[inds],hf[inds]]) # save the z,x,y,h values in order of brightness.

    # only return max_num spots.
    if max_num is not None:
        centers_zxyh = centers_zxyh[:,:max_num]
    return centers_zxyh,std_

def get_N_chrs_per_cell(im_segm,fovnum,zxyh0,dist,max_spots=4):
    """Given a 2d image <im_segm> with cells of different index,and a set of z,x,y,h spot coordinates and brightness <zxyh0>
    This returns the 2 brightest spots that are at least 10 pixels away"""

    # helper which takes a single coordinate and checked if it's at least dist from all accepted pts.
    def check_dist(dist, accepted, cell_ind, zxy_pix_sz=[200,109,109]):

        for spot in accepted:
            if not np.linalg.norm(cell_ind[:3]-spot)>dist:
                return False
        return True
    ...
    for icell in range(1,np.max(im_segm)+1):
        ...
        cell_xy = list(zip(cell_xy[0], cell_xy[1]))
        orig = False #######################################
        first_chr = random.choice(cell_xy) #######################################
        second_chr = random.choice(cell_xy) #######################################

        count = 0
        while orig == False and count < 15: #######################################
            if (math.sqrt((first_chr[0] - second_chr[0])**2 + (first_chr[1] - second_chr[1])**2)) < 15: #######################################
                first_chr = random.choice(cell_xy) #######################################
                second_chr = random.choice(cell_xy) #######################################
            else: #######################################
                orig = True#######################################
        ...
        first_chr = [first_chr[0], first_chr[1], random.randrange(5,65)]#######################################
        second_chr = [second_chr[0], second_chr[1], random.randrange(5,65)] #######################################
        chromosome_array.append((first_chr,second_chr)) #######################################
        ...
        #cell_inds = zxyh0_[keep]
        #brightest_spot = 0

        #has_spots=True

        # get spots until we find one less than half as bright as our brightest
        # spot in each cell.
        #while has_spots:

            # stop if we reach our max number of spots per cell
        #    if (len(cell_zxys) >= max_spots):
        #        has_spots=False
        #        continue

        #    spot_locs = []

            # find all spots which are at least dist away from all currently accepted spots
        #    spot_locs = cell_inds[np.where( [ check_dist(dist, cell_zxys, cell_ind) for cell_ind in cell_inds ] )[0]]
            # ensure that we found some valid spots.

        #    if len(spot_locs) > 0:
                # find the brightest one in the valid spots, save its coords, etc.
        #        ind = np.argmax(spot_locs[:,-1])

                # stop finding new spots if the brightest remaining spot is below 0.6*brightest_spot
        #        if (spot_locs[ind,-1] < 0.4*brightest_spot) and (len(cell_zxys) >= 2):
        #            has_spots=False
        #            break

                # save info of next best spot.
        #        cell_zxys.append(spot_locs[ind,:3])
        #        accepted_spots.append(spot_locs[ind])

                # save cell ids for the spots we identify. to differentiate between FOVS,
                # we number them by 1000*fov*cellnum since we wont have more than 1000 cells per fov ever.
        #        cell_ids.append((1000*fovnum)+icell)

                # record the brightness of brightest spot if we have a spot
        #        brightest_spot = accepted_spots[0][-1]

        #    else:
        #       has_spots=False

    #print(len(accepted_spots) == len(cell_ids))
    #accepted_spots = np.array(accepted_spots).T
    df = pd.DataFrame(chromosome_array)
    df.to_excel("D:\DNA_FISH\E20201013_DSIMU\Analysis_AJ\sim_chroms_rev.xlsx")
    #np.save('D:\DNA_FISH\E20201013_DSIMU\Analysis_AJ\Sim_chromosomes_revised.txt', chromosome_array) #######################################

    return 0, 0

def return_candidate_centers(im,dist_th=20,ex=20,ey=20,ez=5,gmax=5,filtsz=3,ths=4,plt_val=False,save_file=None):
    zs,xs,ys = im.shape
    ...
    keep = (z<zs-ez)&(z>ez)&(x<xs-ex)&(x>ex)&(y<ys-ey)&(y>ey) # make sure that all centers are within the boundaries
    z,x,y,h = z[keep],x[keep],y[keep],h[keep] # subset for only [keep] zxyh values.

    dic = {'zxyh_full':np.array(centersh).T,'zxyh':np.array([z,x,y,h]).T} # save dic of the candidates in the form
    ...
    for ichr,chr_pt in enumerate(chr_pts_):
        ...
        if np.prod(im_sm.shape)>0:
            ...
            while True:
                ...
                center_ = center+mins-txyz_ # transform centers into global coordinates of FOV/round (remove drift correction)
            ...
            # pp are the fits for all candidate spots in centers_keep in the form
            # len(canters_keep) x [ h, x, y, z, w_x, w_y, w_z, background ]
            # where w_x, w_y, and w_z are fixed width at 1 (see fitsinglegaussian_fixed_width)
            pp = fast_local_fit(im_sm,centers_keep)

            # compute the local signal to noise ratio and append to pp for each center.
            for idx in range(len(pp)):

                center_im = grab_block(im_sm, pp[idx,1:4], [20]*3) # take a 15x15x15 block around each spot and compute SNR
                snr = qct.single_spot_local_snr(center_im)
                pp[idx,8] = snr
            ...
            if len(pp)>0:
                pp[:,1:4]=pp[:,1:4]+[mins-txyz_] #apply drift correction to x,y,z coords stores in 2nd to 4th positions of pp array

            #record in dictionary in the old format
        candid_dic,_ = candid_spot.get(ichr,({},None))
        candid_dic[tag]=pp #drift corrected
...
'''
This function takes a list of coordinate positions and computes a moving average
in a window of size n on either side of a given position, ignoring nan values.
'''
def nan_moving_average(a,n=3):
    a_ = np.array(a)
    if n>0: a_ = np.concatenate([a[:n][::-1],a,a[-n:][::-1]])
...
'''
This function takes a list of coordinate positions and computes a moving average
in a window of size n on either side of a given position.
'''
...
'''
This function returns the fraction of elements with value < taget. assumes vals is sorted
'''
...
'''
Function which flattens a list by 1 dimension e.g. 3->2 dim.
'''
def flatten(l):
    return [item for sublist in l for item in sublist]

'''
This function takes a dictionary of candidate spots (dic_cand) and a dictionary of
chromatic mapping functions (dic_chr) along with a chromatic bleedthrough mapping to
transform a set of candidate positions into a corrected position in xyz space.

The basic idea is to transform from one color into a base color. In the case of three
color imaging we transform the high wavelength and low wavelength into the middle wavelength colors,
e.g. 750->647 and 561->647. The function to map these colors to each other is computed
by fitting a large number of colocalizing points in different colors to a quadratic function.

Returns:
candidate dictionary (containing fit parameters, etc for each chromosome across all rounds)
with coordinates corrected for colored effects.
'''
def correct_color(dic_cand,htags,dic_chr={},dic_bleed={},
                  zxy_pix_size = [200,109,109]):

    # helper functions to grab information about candidate spots
    ...
    # apply the color correction to each spot in each round
    for htag in htags:
        col = get_col(htag)
        ...
        if len(ps)==0:
            ...
            hs = ps[:,0]
            # this step is purely for utility, reformatting the parameter lists for ease of access
            dic_cand_[htag] = {'ps':ps,'hs':hs,'zxys':zxys}

    # apply the breedthrough correction
    if len(dic_bleed)>0:
...
'''
This function takes a set of parameters and computes a statistical estimate similar
to a fischer's exact p value based upon the relative ranking of a given point among all
currently accepted points. It has been modified to take supplied weights for the scores
which determin the relative importance of each parameter in the assessment of scores
for iterative refinement of the accepted set of points.

Returns: The list of coordinates which recieved the highest score for each chromosomes
for each imaging round along with thei associated parameter lists.
'''
def get_statistical_estimate(dic_cand,htags,Dhs,Ddists,DdistsC,Dsnrs,zxys_T=None,nint=5,use_local=1,use_center=1,use_snr=1,use_brightness=1):

    # initalize with brightest if we havent initialized yet
    if zxys_T is None:
        zxys_T,hs_T,cols_T,snrs_T = get_maxh_estimate(dic_cand,htags)
    ...
    # go through each round for the given chromosome
    for u_i in range(len(htags)):
        ...
        snrs = np.array([dic_cand[htag]['ps'][i,8] for i in range(len(dic_cand[htag]['ps'])) ])

        if len(zxys)==0:
            zxys_T.append([np.nan]*3)
            hs_T.append(np.nan)
            scores_T.append(np.nan)
            snrs_T.append(np.nan)
            continue

        dists = np.linalg.norm(zxys-zxys_mv[u_i],axis=-1)
        distsC = np.linalg.norm(zxys-zxysC,axis=-1)
        total_weight = use_local + use_center + use_snr + use_brightness
        #scores = [(use_center*(1-cum_val(DdistsC,dC_))+use_local*(1-cum_val(Ddists,d_))+use_brightness*cum_val(Dhs,h_)+use_snr*cum_val(Dsnrs,s_))/total_weight for dC_,d_,h_,s_ in zip(distsC,dists,hs,snrs)]
        if use_local and use_center and use_snr:
            scores = [(1-cum_val(DdistsC,dC_))*(1-cum_val(Ddists,d_))*(cum_val(Dhs,h_))*cum_val(Dsnrs,s_) for dC_,d_,h_,s_ in zip(distsC,dists,hs,snrs)]
        if use_local and use_center and not use_snr:
            scores = [(1-cum_val(DdistsC,dC_))*(1-cum_val(Ddists,d_))*(cum_val(Dhs,h_)) for dC_,d_,h_,s_ in zip(distsC,dists,hs,snrs)]
        if not use_local and use_center and use_snr:
            scores = [(1-cum_val(DdistsC,dC_))*(cum_val(Dhs,h_))*cum_val(Dsnrs,s_) for dC_,d_,h_,s_ in zip(distsC,dists,hs,snrs)]
        if use_local and not use_center and use_snr:
            scores = [(1-cum_val(Ddists,d_))*(cum_val(Dhs,h_))*cum_val(Dsnrs,s_) for dC_,d_,h_,s_ in zip(distsC,dists,hs,snrs)]
        if not use_local and not use_center and use_snr:
            scores = [cum_val(Dhs,h_)*cum_val(Dsnrs,s_) for dC_,d_,h_,s_ in zip(distsC,dists,hs,snrs)]
        iscore = np.argmax(scores)
        all_scores.append(scores)
        scores_T.append(scores[iscore])
        zxys_T.append(zxys[iscore])
        hs_T.append(hs[iscore])
        snrs_T.append(snrs[iscore])

    zxys_T = np.array(zxys_T)
    hs_T =np.array(hs_T)
    cols_T = np.array(cols_T)
    snrs_T = np.array(snrs_T)

    return zxys_T,hs_T,cols_T,scores_T,snrs_T,all_scores

'''
This function takes the set of points in the current iteration of the EM algorithm which
are considered 'best' or accepted and then orders the points by each variable, in this instance
we are using distance, brightness, and moving average. (NO SNR)
Returns: ordered list of heights, distances from chr center, and distance from moving averages.
'''
...
This function takes the set of points in the current iteration of the EM algorithm which
are considered 'best' or accepted and then orders the points by each variable, in this instance
we are using snr, distance, brightness, and moving average.
Returns: ordered list of heights, distances from chr center, distance from moving average, and
signal to noise ratios.
'''
def get_Ddists_Dhs_Dsnrs(zxys_f,hs_f,snrs_f,nint=5):
    # unravel hs and sort
    h = np.ravel(hs_f)
    h = h[np.isnan(h)==False]
    h = np.sort(h)

    # unravel and sort snrs as well
    s = np.ravel(snrs_f)
    s = s[np.isnan(s)==False]
    s = np.sort(s)
    ...
    # compute the
    for zxys_T in zxys_f:
        difs = zxys_T-nan_moving_average(zxys_T,nint)
        difsC = zxys_T-np.nanmedian(zxys_T,axis=0)
        dists.extend(np.linalg.norm(difs,axis=-1))
        distsC.extend(np.linalg.norm(difsC,axis=-1))
    dists = np.array(dists)
    ...
    return h,dists,distsC,s

'''
This function takes the candidate points and returns the indices of the spots which
have the highest BRIGHTNESS for each chromosome for each round of imaging. Typically used
to initialize the algorithm.
Returns: a list of zxy positions, heights, snrs, colors for the highest BRIGHTNESS points
of each chromosome in each round of imaging.
'''
def get_maxh_estimate(dic_cand,htags):
    ...
    for htag in htags:
        ...
        snrs_T.append(ps[imax,8])
    return zxys_T,hs_T,cols_T,snrs_T

'''
This function takes the candidate points and returns the indices of the spots which
have the highest SNR for each chromosome for each round of imaging. Typically used
to initialize the algorithm.
Returns: a list of zxy positions, heights, snrs, colors for the highest SNR points
of each chromosome in each round of imaging.
'''
def get_maxSnr_estimate(dic_cand,htags):
    zxys_T = []
    hs_T=[]
    cols_T=[]
    snrs_T=[]

    # take htags for each round and go through one at a time.
    for htag in htags:
        col = htag.split('_')[-1]
        cols_T.append(col)

        # pull out important quantities for later use.
        hs = dic_cand[htag]['hs']
        zxys = dic_cand[htag]['zxys']
        snrs = np.array([dic_cand[htag]['ps'][i,8] for i in range(len(dic_cand[htag]['ps'])) ])
        ...
        # pull out the values for each parameter at the higher snr location.
        imax = np.argmax(snrs) # determine the max value for snr and use its index
        hs_T.append(hs[imax])
        zxys_T.append(zxys[imax])
        snrs_T.append(snrs[imax])
