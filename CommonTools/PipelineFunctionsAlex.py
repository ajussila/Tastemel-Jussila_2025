...
def get_candidate_spots(analysis_folder, colors):
    ...
    for dax_round in tqdm(list(zip(*daxs))):
        ...
        # this only looks at spots identified in the retrun_candidate_centers above.
...
def get_candidate_spots_new(analysis_folder, colors):
    ...
    for ims in tqdm(ims_segmented):#dax_round in tqdm(list(zip(*daxs))):
        ...
        zxyh_keep, cell_ids = ft.get_N_chrs_per_cell(im_segm, isegm, np.concatenate(zxyhs,axis=-1),num_pts = 4, dist = 30)
...
def em_algorithm(analysis_folder, colors, chromatic_correction=None, use_center=1):
    ## Read files ############
    ...
    ### Run to converge #########
    def refine_set(dic_spots,zxys_f,hs_f, use_local=1,use_center=1,use_snr=1,use_brightness=1):
        #print("Lengths dics,zxys:",np.sum(list(map(len,dic_spots))),len(zxys_f))
        Dhs,Ddists,DdistsC,Dsnrs = ft.get_Ddists_Dhs_Dsnrs(zxys_f,hs_f,snrs_f,nint=5)
    ...
    #for num_ref in range(niter):
        ...
        use_snr = 1
        use_brightness = 1
        zxys_f,hs_f,cols_f,scores_f,snrs_f,all_scores_f = refine_set(dic_spots_cor,zxys_f,hs_f,use_local=use_local,use_center=use_center,use_snr=use_snr,use_brightness=use_brightness)
        ...
        num_ref += 1
    return zxys_f, hs_f, cols_f, snrs_f, scores_f, all_scores_f, dic_spots, dic_drifts, dic_cell_ids, fovs_spots
...
def filter_chromosomes(num_segs, max_missing, zxys_f, hs_f, cols_f, scores_f, all_scores_f, dic_cell_ids, fovs_spots, cutoff_exp):
    ...
    print([len(dic) for dic in dic_cell_ids])
    print(len(zxys_f))
    cell_ids_clean = np.array(ft.flatten(dic_cell_ids))
    print(len(cell_ids_clean))
