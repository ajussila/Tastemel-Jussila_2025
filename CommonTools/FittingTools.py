...
def fast_local_fit(im,centers,radius=5,width_zxy=[1,1,1],return_good=False):
    ...
    for center in centers:
        ...
        # check if the fit location is within the acceptable radius to determine if it is a good fit (if so, change to True).
        if p is not None:
...
def get_seed_points_base_v2(im_sm,gfilt_size=5,filt_size=3,th_seed=3.,max_num=None):
    ...
    #
    z,x,y = np.where(im_norm>hcutoff) # find any positions where the brightness exceeds this value.
...
def get_N_chrs_per_cell(im_segm,fovnum,zxyh0,dist,max_spots=3):
    ...
    def check_dist(dist, accepted, cell_ind):
    ...
    return accepted_spots, cell_ids
...
'''
Same as above, except the SNR parameter has been removed.
'''
def get_statistical_estimate_no_snr(dic_cand,htags,Dhs,Ddists,DdistsC,zxys_T=None,nint=5,use_local=True,use_center=True):
    if zxys_T is None:
...
have the highest BRIGHTNESS for each chromosome for each round of imaging. Typically used
to initialize the algorithm.
Returns: a list of zxy positions, heights, colors for the highest BRIGHTNESS points
of each chromosome in each round of imaging.
'''
def get_maxh_estimate_no_snr(dic_cand,htags):
    zxys_T = []
    hs_T=[]
    cols_T=[]
